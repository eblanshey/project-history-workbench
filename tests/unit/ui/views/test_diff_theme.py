...
import pytest
from freecad.diff_wb.qt import QtGui
...
def test_light_theme_uses_original_bright_backgrounds_with_black_text(
    state: DiffState, expected: QtGui.QColor
) -> None:
    """Light themes keep original bright highlight colors and black text."""
    palette = _palette(
        base=QtGui.QColor(255, 255, 255), text=QtGui.QColor(0, 0, 0), window=QtGui.QColor(245, 245, 245)
    )
...
def test_dark_theme_uses_blended_background_instead_of_light_pastel() -> None:
    ...
    palette = _palette(
        base=QtGui.QColor(30, 30, 30), text=QtGui.QColor(240, 240, 240), window=QtGui.QColor(20, 20, 20)
    )
